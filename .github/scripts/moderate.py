...
下面依次给出:全站登记表 comics.json 全文(其中大部分条目是已上线的旧作品,仅供\
你核对新增条目的文本,旧条目及其未随附的图片不在本次审核范围内,不构成拒绝理由),\
以及本次 PR 新增/修改的每张图片(文件名在图片前标注)——只需审核这些图片和与之\
同目录的新增登记条目。reasons 用中文,逐条写明命中的标准和涉及的文件;全部通过\
时 reasons 为空数组。"""
